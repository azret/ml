...
#define EPSILONE 1e-12
    for (unsigned int t = 0; t < 8; t++) {
        double u1 = 1 - data[t];
        double u2 = data[t + 8];
        double radius = sqrt(-2 * log(u1 + EPSILONE));
        double theta = 2.0 * M_PI * u2;
        data[t] = (float)(radius * cos(theta) * std + mean);
        data[t + 8] = (float)(radius * sin(theta) * std + mean);
    }
}
...
#define EPSILONE 1e-12
    if (numel >= 16) {
        ...
        for (unsigned int t = 0; t < numel; t++) {
            ...
            double u1 = randfloat64(state);
            double u2 = randfloat64(state);
            double radius = sqrt(-2 * log(1 - u2 + EPSILONE));
